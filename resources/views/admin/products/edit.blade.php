                ...
                <div class="col">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <!-- Multi Columns Form -->
                            <form method="post" action="{{route('products.update', ['product'=>$editProduct->id])}}" class="row g-3" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
{{--                                <input type="hidden" name="product_id" value="{{$editProduct->id}}">--}}
                                    ...
                                    <div class="col">
                                        <select class="form-select" aria-label="Default select example" name="category_id">
                                            @foreach($productCategories as $key => $value)
                                                <option value="{{$value->id}}" {{($editProduct->category_id) == $value->id ? "selected" : ""}}>{{$value->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="col">
                                        <select class="form-select" aria-label="Default select example" name="brand_id">
                                            @foreach($brands as $key => $value)
                                                <option value="{{$value->id}}" {{($editProduct->brand_id) == $value->id ? "selected" : ""}}>{{$value->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-12">
                                    ...
                                    <!-- TinyMCE Editor -->
                                    <textarea class="tinymce-editor" id="description" name="description">{{$editProduct->description}}</textarea><!-- End TinyMCE Editor -->
                                </div>
